...
class ParallelJRCProcessor:
    ...
    def __init__(self, grid_csv_path, output_dir="data/outputs"):
        ...
        print(f"   💻 CPU: {self.cpu_count} physical, {self.cpu_logical} logical cores")
        print(f"   🧠 Memory: {self.memory_gb:.1f} GB")
        print(f"   ⚡ Workers: {self.max_workers} (optimized for on-the-go processing)")
    ...
    def load_grid_data(self):
        ...
        print(f"\n📊 Loading grid data from: {self.grid_csv_path}")
        
        try:
            self.grid_df = pd.read_csv(self.grid_csv_path)
            print(f"   ✅ Loaded {len(self.grid_df):,} grid cells")
            ...
            print(f"   🗺️ Available tiles: {len(self.available_tiles)}")
            print(f"   📋 Sample tiles: {self.available_tiles[:5]}{'...' if len(self.available_tiles) > 5 else ''}")
    ...
    def download_jrc_tile(self, tile_id, dataset_type):
        ...
        print(f"    📥 Downloading {dataset_type.upper()} tile {tile_id}...")
...
def process_grid_chunk_optimized(args):
    ...
    Uses on-the-go grid calculation with class counts
    """
    grid_chunk, raster_path, dataset_type, cell_size, class_mappings = args
            ...
            for _, grid_row in grid_chunk.iterrows():
                try:
                    ...
                    if len(valid_data) > 0:
                        ...
                        for class_value, class_name in class_mappings.items():
                            safe_class_name = class_name.replace(' ', '_').replace('<', 'lt').replace('>', 'gt').replace('=', 'eq').replace(',', '').replace('(', '').replace(')', '')
                            grid_result[f'{dataset_type}_count_{class_value}_{safe_class_name}'] = 0
                        
                        # Fill in actual counts for classes that exist
                        for val, count in zip(unique_vals, counts):
                            if val in class_mappings:
                                class_name = class_mappings[val]
                                safe_class_name = class_name.replace(' ', '_').replace('<', 'lt').replace('>', 'gt').replace('=', 'eq').replace(',', '').replace('(', '').replace(')', '')
                                grid_result[f'{dataset_type}_count_{val}_{safe_class_name}'] = int(count)
                        
                        # Add total pixel count and coverage
                        grid_result[f'{dataset_type}_total_pixels'] = len(valid_data)
                        grid_result[f'{dataset_type}_coverage'] = (len(valid_data) / (data.size)) * 100
                        
                        # Add dominant class info
                        dominant_idx = np.argmax(counts)
                        dominant_class = int(unique_vals[dominant_idx])
                        grid_result[f'{dataset_type}_dominant_class'] = dominant_class
                        grid_result[f'{dataset_type}_dominant_count'] = int(counts[dominant_idx])
                        grid_result[f'{dataset_type}_dominant_percentage'] = (counts[dominant_idx] / len(valid_data)) * 100
                        
                    else:
                        # No valid data - initialize all counts to 0
                        for class_value, class_name in class_mappings.items():
                            safe_class_name = class_name.replace(' ', '_').replace('<', 'lt').replace('>', 'gt').replace('=', 'eq').replace(',', '').replace('(', '').replace(')', '')
                            grid_result[f'{dataset_type}_count_{class_value}_{safe_class_name}'] = 0
                        
                        grid_result[f'{dataset_type}_total_pixels'] = 0
                        grid_result[f'{dataset_type}_coverage'] = 0
                        grid_result[f'{dataset_type}_dominant_class'] = np.nan
                        grid_result[f'{dataset_type}_dominant_count'] = 0
                        grid_result[f'{dataset_type}_dominant_percentage'] = 0
                    ...
                    # Initialize all class counts to 0 for error cases
                    for class_value, class_name in class_mappings.items():
                        safe_class_name = class_name.replace(' ', '_').replace('<', 'lt').replace('>', 'gt').replace('=', 'eq').replace(',', '').replace('(', '').replace(')', '')
                        error_result[f'{dataset_type}_count_{class_value}_{safe_class_name}'] = 0
                    
                    error_result[f'{dataset_type}_total_pixels'] = 0
                    error_result[f'{dataset_type}_coverage'] = 0
                    error_result[f'{dataset_type}_dominant_class'] = np.nan
...
class ParallelJRCProcessor:
    ...
    def process_tile_parallel(self, tile_id, datasets=['built_c'], aggregations=['mean', 'max', 'dominant'], 
        ...
        if len(tile_grids) == 0:
            print(f"  ⚠️ No grids found for tile {tile_id}")
            return None
        
        print(f"  📊 {len(tile_grids):,} grids to process")
        
        tile_results = []
        
        for dataset in datasets:
            print(f"  🗂️ Processing {dataset.upper()} dataset...")
            ...
            print(f"    ✅ Raster available: {raster_path.name}")
            ...
            print(f"    📦 Split into {len(chunks)} chunks of ~{chunk_size} grids each")
            ...
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(future_to_chunk):
                    ...
                    try:
                        ...
                        print(f"      📈 Progress: {completed_chunks}/{len(chunks)} chunks ({progress:.1f}%)")
            ...
            print(f"    ✅ Success rate: {len(valid_results):,}/{len(tile_grids):,} ({success_rate:.1f}%)")
            print(f"    🚀 Speed: {len(tile_grids)/processing_time:.1f} grids/second")
        ...
        if tile_results:
            ...
            print(f"  📊 Final results: {len(final_results):,} grids with {len(final_results.columns)} columns")
    ...
    def process_all_tiles(self, datasets=['built_c'], aggregations=['mean', 'max', 'dominant'], 
        ...
        print(f"🗺️ Total tiles: {len(self.available_tiles)}")
        print("=" * 80)
            ...
            try:
                ...
                if tile_result is not None:
                    ...
                    if save_individual:
                        ...
                        print(f"  💾 Saved tile results: {tile_file}")
                
                else:
                    failed_tiles.append(tile_id)
                    print(f"  ❌ Tile {tile_id} processing failed")
        ...
        if all_results:
            print(f"\n📊 Combining results from {len(all_results)} tiles...")
            ...
            for dataset in datasets:
                if f'{dataset}_dominant' in combined_results.columns:
                    if dataset == 'built_c':
                        built_mapping = {
                            11: "Residential ≤3m", 12: "Residential 3-6m", 13: "Residential 6-15m",
                            14: "Residential 15-30m", 15: "Residential >30m", 21: "Non-residential ≤3m",
            ...
            print(f"🗺️ Successful tiles: {successful_tiles}/{len(self.available_tiles)} ({(successful_tiles/len(self.available_tiles))*100:.1f}%)")
            print(f"❌ Failed tiles: {failed_count} - {failed_tiles[:3]}{'...' if len(failed_tiles) > 3 else ''}")
            print(f"🚀 Processing speed: {total_grids/total_time:.1f} grids/second")
            print(f"💾 Final results: {csv_file}")
            ...
            for dataset in datasets:
                if f'{dataset}_pixel_count' in combined_results.columns:
                    ...
                    print(f"📈 {dataset.upper()} coverage: {valid_grids:,} grids ({coverage:.1f}%)")
                    
                    if f'{dataset}_mean' in combined_results.columns:
                        mean_value = combined_results[f'{dataset}_mean'].mean()
                        print(f"📊 {dataset.upper()} average: {mean_value:.3f}")
            
            # Sample results
            print(f"\n📋 Sample Results (first 5 grids):")
            ...
            print(f"\n❌ No successful results from any tiles")
            return None
...
def main():
    ...
    print("🌍 PARALLEL JRC RASTER AGGREGATION")
    print("🚀 Optimized for on-the-go grid calculations")
    print("=" * 60)
    
    # Check if grid file exists
    if not os.path.exists(GRID_CSV_PATH):
        print(f"❌ Grid CSV file not found: {GRID_CSV_PATH}")
    ...
    try:
        ...
        # Show configuration
        print(f"\n🎯 PROCESSING CONFIGURATION:")
        print(f"   📁 Grid file: {GRID_CSV_PATH}")
        print(f"   📊 Datasets: {DATASETS}")
        print(f"   🔢 Aggregations: {AGGREGATIONS}")
        print(f"   📏 Cell size: {CELL_SIZE}m")
        ...
        if results is not None:
            print(f"\n✅ SUCCESS! Processed {len(results):,} grid cells")
